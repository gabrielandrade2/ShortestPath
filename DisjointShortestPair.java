package DisjointShortestPair;

import static java.sql.Types.NULL;
import java.util.ArrayList;
import java.util.Arrays;

public class DisjointShortestPair {
   ...
   static final int inf = 999;
    ...
    public static void main(String[] args) {
        ...
        //2nd step
        for(int i=0; i<firstShortestPath.size()-1; i++){    //Reversing the directions of the links
            int a = firstShortestPath.get(i);               //and setting negative costs
            int b = firstShortestPath.get(i+1);
            modGraph[b][a] = graph[a][b] * -1;
            modGraph[a][b] = graph[b][a];
        }
        //3rd step
        ArrayList<Integer> secondShortestPath = BellmanFord(modGraph, startNode, endNode);
        if(secondShortestPath == null){     //In case there is an error
     ...
     public static ArrayList<Integer> BellmanFord(int[][] graph, int startNode, int endNode) {
       //Represents the graph with the costs between each link
       int[][] costs = graph;
       ...
       //initialize with the start node, detting its distance to 0
        ...
        for(int i=0; i<n; i++){
            for(int j=0; j<costs.length; j++){
                ...
                if(dist < distance[j]){
                    System.out.println("Graph contais a negative-cost loop");
                    return null;
                }
            }
        }
